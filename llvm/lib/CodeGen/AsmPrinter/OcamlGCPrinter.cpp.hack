...
GCMetadataPrinterRegistry::Add<OcamlGCMetadataPrinter>
    Y("ocaml", "ocaml 5-compatible collector");
...
void OcamlGCMetadataPrinter::finishAssembly(Module &M, GCModuleInfo &Info,
  ...
  for (std::unique_ptr<GCFunctionInfo> &FI :
    ...
    for (GCFunctionInfo::iterator J = FI->begin(), JE = FI->end(); J != JE;
      ...
      // CR yusumez: Figure out a way to access the current line
      // (or a better way to emit temp symbols)
      MCSymbol *ThisSymbol = EmitFrametableSymbol(M, AP, J->Label->getName());
      AP.OutStreamer->emitAbsoluteSymbolDiff(J->Label, ThisSymbol, 4);
